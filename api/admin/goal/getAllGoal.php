...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    $params[] = $status;
    $paramString = "s";
    $sortQuery = ' AND ' . $goalDBCall::tableName . '.status = ?';
    $searchQuery = "";
    $single_post = false;
    ...
    if (isset($_GET['trackid'])) {
        ...
        $sortQuery .= ' AND ' . $goalDBCall::tableName . '.trackid = ?';
        $paramString .= "s";
        $params[] = $status;
    } else {
        $single_post = false;
    ...
    if (isset($_GET['search'])) {
        $tableName = $goalDBCall::tableName;
    ...
    $allEvents = $goalDBCall::getAllGoal($page_no, $offset, $noPerPage, $searchQuery, $sortQuery, $paramString, $params);
