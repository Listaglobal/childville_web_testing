...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $goal = " ";
    if (isset($_POST['goal'])) {
        $goal = $utility_class_call::escape($_POST['goal']);
    }

    // checking all paramater are passed

    if ( $utility_class_call::validate_input($user_id) || $utility_class_call::validate_input($goal)
    ) {
    ...
    $StaffReview = $goalDBCall::addGoal($data);
    ...
    $maindata = [];
    $text = $api_response_class_call::$staffGoal;
