...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    $params[] = $status;
    $paramString = "s";
    $sortQuery = ' AND ' . $pupilsDBCall::tableName . '.status = ?';
    $searchQuery = "";
    $single_post = false;
    ...
    if (isset($_GET['trackid'])) {
        ...
        $sortQuery .= ' AND ' . $pupilsDBCall::tableName . '.trackid = ?';
        $paramString .= "s";
        $params[] = $status;
    } else {
        $single_post = false;
    ...
    if (isset($_GET['search'])) {
        ...
        $statesTable = $discoDBCall::$statesTable;
        $search = $utility_class_call::escape($_GET['search']);
        if (!empty($search) && $search != '' && $search != " ") {
            $searchValue = "%" . $search . "%";
            $searchQuery = " AND ( $tableName.fullName LIKE ? OR $tableName.parentName LIKE ? OR $tableName.parentEmail OR $tableName.pcontant )";
            for ($i = 0; $i < 2; $i++) {
                $params[] = $searchValue;
                $paramString .= "s";
            }
    ...
    $allPupils = $pupilsDBCall::getallPupils($page_no, $offset, $noPerPage, $searchQuery, $sortQuery, $paramString, $params);
