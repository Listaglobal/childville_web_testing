...
// 405 Method Not Allowed
    ...
    function respondOKButFailed($maindata, $text, $hint, $linktosolve, $errorcode)
    {
        $method = getenv('REQUEST_METHOD');
        $endpoint = Utility_Functions::getCurrentFullURL();
    ...
    function getTokenToSendAPI($userPubkey,$longer=0)
    {
    ...
    function ValidateAPITokenSentIN()
    {
        ...
        try {
            ...
            $token = JWT::decode($jwt, new Key($secretKey, 'HS512'));//['HS512']
    ...
    function getFastTokenToSendAPI($userPubkey)
    {

        $systemData = System_Defaults::getAPIJwtData();
        $companyprivateKey = $systemData['privatekey'];
        $minutetoend = $systemData['fast_api_end_time'];
        $serverName = $systemData['fast_api_server_name'];
    ...
    function ValidateFastAPITokenSentIN()
    {
        $method = getenv('REQUEST_METHOD');
        $endpoint = Utility_Functions::getCurrentFullURL();
        $systemData = System_Defaults::getAPIJwtData();
